    ...
    try {
        ...
        for (const employeeId of ids) {
                ...
                return {
                    ...
                    meta: {
                        ...
                        address: `${serviceInfo?.address || ''} ${
                            ...
                            serviceInfo?.postCode ? ` ${serviceInfo.postCode}` : ''
                        }`.trim(),
                        category: serviceInfo?.type || '',
                        description:
                            serviceInfo?.comments || serviceInfo?.type || '',
                    },
                    rows: [
                        {
                            name: entry.name,
                            shifts: Object.fromEntries(
                                Object.entries(entry.shifts).map(
                                    ([key, value]) => [key, value.join('\n')]
                                )
                            ),
                            hoursByDay: Object.fromEntries(
                                Object.entries(entry.hoursByDay).map(
                                    ([key, value]) => [
                                        key,
            ...
            const filePath = await createScheduleGridPdfUtil({
                ...
                fileName: `personal-${safeName}-${effectiveMonth}.pdf`,
            });
