...
const initSocket = (httpServer) => {
    ...
    io.use(async (socket, next) => {
        try {
            ...
            if (!token) return next(new Error('Se esperaba un token'));
            const payload = jwt.verify(token, SECRET);
            const user = await selectUserByIdService(payload.id);
            if (!user.active) return next(new Error('Usuario pendiente de activacion'));
    ...
    io.on('connection', (socket) => {
        ...
        const isAdminUser = socket.user?.role === 'admin' || socket.user?.role === 'sudo';

        socket.join(`user:${socket.user.id}`);
        if (isAdminUser) socket.join('admins');
        ...
        socket.on('chat:message', async ({ serviceId, message, imagePath, replyToMessageId }, callback) => {
            try {
                const text = message ? String(message).trim() : '';
                const image = imagePath ? String(imagePath).trim() : '';
                if (!text && !image) generateErrorUtil('Mensaje vacio', 400);
                await ensureServiceChatAccessService(serviceId, socket.user.id, socket.user.role);
        ...
        socket.on('chat:pause', async ({ serviceId, paused }, callback) => {
            try {
                ...
                await ensureServiceChatAccessService(serviceId, socket.user.id, socket.user.role);
                const nextPaused = await updateServiceChatPausedService(serviceId, Boolean(paused));
                io.to(`service:${serviceId}`).emit('chat:pause', { serviceId, paused: nextPaused });
        ...
        socket.on('chat:delete', async ({ serviceId, messageId }, callback) => {
            try {
                if (!isAdminUser) generateErrorUtil('Acceso denegado: Se requiere rol de Administrador', 403);
                await ensureServiceChatAccessService(serviceId, socket.user.id, socket.user.role);
                await deleteServiceChatMessageService(serviceId, messageId);
                io.to(`service:${serviceId}`).emit('chat:delete', { serviceId, messageId });
                callback?.({ ok: true });
            } catch (error) {
                callback?.({ ok: false, message: error.message });
            }
        ...
        socket.on('chat:clear', async ({ serviceId }, callback) => {
            try {
                if (!isAdminUser) generateErrorUtil('Acceso denegado: Se requiere rol de Administrador', 403);
                await ensureServiceChatAccessService(serviceId, socket.user.id, socket.user.role);
                await deleteServiceChatMessagesByServiceService(serviceId);
                io.to(`service:${serviceId}`).emit('chat:clear', { serviceId });
                callback?.({ ok: true });
            } catch (error) {
                callback?.({ ok: false, message: error.message });
            }
        ...
        socket.on('generalChat:message', async ({ chatId, message, imagePath, replyToMessageId }, callback) => {
            try {
                const text = message ? String(message).trim() : '';
                const image = imagePath ? String(imagePath).trim() : '';
                if (!text && !image) generateErrorUtil('Mensaje vacio', 400);
                await ensureGeneralChatAccessService(chatId, socket.user.id);
                await ensureGeneralChatWriteAccessService(chatId, socket.user.role);
        ...
        socket.on('generalChat:delete', async ({ chatId, messageId }, callback) => {
            try {
                if (!isAdminUser) generateErrorUtil('Acceso denegado: Se requiere rol de Administrador', 403);
                await ensureGeneralChatAccessService(chatId, socket.user.id);
                await deleteGeneralChatMessageService(chatId, messageId);
                io.to(`generalChat:${chatId}`).emit('generalChat:delete', { chatId, messageId });
                callback?.({ ok: true });
            } catch (error) {
                callback?.({ ok: false, message: error.message });
            }
